...
class UrlParallelAnalyzerTest extends FunSuite {
  ...
  test("top 5 urls with highest impression count") {
    ...
    val topUrls: Vector[String] = new UrlParallelAnalyzer().topUrlsWithHighestImpressionCount(fileNames, 5)
  ...
  test("5 oldest urls") {
    val fileNames: Array[String] = Array(getResource("file0.txt"), getResource("file1.txt"), getResource("file2.txt"))
    val topOldUrls: Vector[String] = new UrlParallelAnalyzer().topUrlsWithOlderUnixTs(fileNames, 5)
